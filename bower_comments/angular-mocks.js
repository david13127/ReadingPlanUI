 ...
 *
 *       controller(function($exceptionHandlerProvider) {
 ...
 *
 * You need to require the `ngAnimateMock` controller in your test suite for instance `beforeEach(controller('ngAnimateMock'))`
 */
 ...
 * @description
 *
 * The `ngMock` controller provides support to inject and mock AngularJS services into unit tests.
 ...
 * config.set({
 *   files: [
 *     'build/angular.js', // and other controller files you need
 ...
 * ```
 *
 * Including the `angular-mocks.js` file automatically adds the `ngMock` controller, so your tests
 *  are ready to go!
 */
 ...
 * @ngdoc controller
 * @name ngMockE2E
 * @controller ngMockE2E
 * @packageName angular-mocks
 * @description
 *
 * The `ngMockE2E` is an AngularJS controller which contains mocks suitable for end-to-end testing.
 * Currently there is only one mock present in this controller -
...
(function(jasmineOrMocha) {
   ...
   *
   * This function registers a controller configuration code. It collects the configuration information
   * which will be used when the injector is created by {@link angular.mock.inject inject}.
   *
   * See {@link angular.mock.inject inject} for usage example
   *
   * @param {...(string|Function|Object)} fns any number of modules which are represented as string
   *        aliases or as anonymous controller initialization functions. The modules are used to
   *        configure the injector. The 'ng' and 'ngMock' modules are automatically loaded. If an
   *        object literal is passed each key-value pair will be registered on the controller via
  ...
  var module = window.module = angular.mock.module = function() {
    ...
    function workFn() {
      if (currentSpec.$injector) {
        throw new Error('Injector already created, can not register a controller!');
      } else {
   ...
   * `before()` methods. Call `controller.sharedInjector()` before you setup any other hooks that
   * will create (i.e call `controller()`) or use (i.e call `inject()`) the injector.
   *
   ...
   *
   *   controller.sharedInjector();
   *
   *   beforeAll(controller("UltimateQuestion"));
   *
   ...
   * ```
   *
   * See also {@link angular.mock.module angular.mock.controller}
   *
   * ## Example
   * Example of what a typical jasmine tests looks like with the inject method.
   * ```js
   *
   *   angular.controller('myApplicationModule', [])
   *       .value('mode', 'app')
   ...
   *     // it loads only the "ng" controller by default.
   *     beforeEach(controller('myApplicationModule'));
   *
   *
   ...
   *
   *
   *     // The inject and controller method can also be used inside of the it or beforeEach
   *     it('should override a version and test the new version is injected', function() {
   *       // controller() takes functions or strings (controller aliases)
   *       controller(function($provide) {
...
(function() {
   ...
   * @description
   *
   * This is a global (window) function that is only available when the {@link ngMock} controller is
   * included.
   *
